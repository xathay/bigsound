...
unsafe extern "C" fn run(handle: *mut c_void, sample_count: c_ulong) {
    ...
    let amount = unsafe { read_control(inst.port_amount, 0.0) };
    let cutoff = unsafe { read_control(inst.port_cutoff, 700.0) };
    let delay = unsafe { read_control(inst.port_delay, 280.0) };
